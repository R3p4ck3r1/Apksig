...
public class ApkSigner {
    ...
    private void sign(DataSource inputApk, DataSink outputApkOut, DataSource outputApkIn)
        ...
        if (outputApkSigningBlockRequest != null) {
            ...
            long fileSize = outputCentralDirStartOffset + outputCentralDirDataSource.size()
                    + padding + outputApkSigningBlock.length + outputEocd.remaining();
            if (mAlignFileSize && (fileSize % ANDROID_FILE_ALIGNMENT_BYTES != 0)) {
                int eocdPadding = (int) (ANDROID_FILE_ALIGNMENT_BYTES - fileSize % ANDROID_FILE_ALIGNMENT_BYTES);
                // Replace EOCD with padding one so that output file size can be the multiples of alignment.
                ByteBuffer newEocd = ByteBuffer.allocate((int) outputEocd.remaining() + eocdPadding);
                newEocd.order(ByteOrder.LITTLE_ENDIAN);
                newEocd.put(outputEocd);
                newEocd.rewind();
                ZipUtils.updateZipEocdCommentLen(newEocd);
                outputEocd = newEocd;
    ...
    public static class Builder {
        ...
        /**
         * Sets whether the output APK files should be sized as mulitples of 4K.
         *
         * <p><em>Note:</em> This method may only be invoked when this builder is not initialized
         * with an {@link ApkSignerEngine}.
         *
