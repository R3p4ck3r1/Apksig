...
public class ApkVerifier {
    ...
    private Result verify(DataSource apk)
        ...
        System.out.println("targetSandboxVersion: " + targetSandboxVersion);
        if (targetSandboxVersion > 1) {
            // APK Signature Scheme v2 signature required for apps targeting sandbox version 2 and
            // higher
            if (foundApkSigSchemeIds.isEmpty()) {
                result.addError(
                        Issue.MISSING_SIG_REQUIRED_FOR_TARGET_SANDBOX_VERSION,
    ...
    private static ByteBuffer getAndroidManifestFromApk(
            DataSource apk, ApkUtils.ZipSections zipSections)
                    throws IOException, ZipFormatException, ApkFormatException {
        List<CentralDirectoryRecord> cdRecords =
                V1SchemeVerifier.parseZipCentralDirectory(apk, zipSections);
        ByteBuffer result =
                ApkSigner.getAndroidManifestFromApk(
                        cdRecords,
     ...
     * AndroidManifest.xml.
     */
    private static final int TARGET_SANDBOX_VERSION_ATTR_ID = 0x0101054e;
    ...
    public static enum Issue {
         ...
         * </ul>
         */
        MISSING_SIG_REQUIRED_FOR_TARGET_SANDBOX_VERSION(
