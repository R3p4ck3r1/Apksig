...
public abstract class V2SchemeSigner {
    ...
    private static byte[] generateApkSigningBlock(byte[] apkSignatureSchemeV2Block) {
        ...
        //     uint32:           ID
        //     (size - 4) bytes: value
        // (extra dummy ID-value for padding to make up 4096 bytes for the whole block)
        ...
        if (resultSize % ANDROID_COMMON_PAGE_ALIGNMENT_BYTES != 0) {
            ...
            if (padding < 12) {  // size of an ID-value pair
                padding += ANDROID_COMMON_PAGE_ALIGNMENT_BYTES;
            }
            paddingPair = ByteBuffer.allocate(padding);
