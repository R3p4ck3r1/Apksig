...
public class ChainedDataSource implements DataSource {
    ...
    public void feed(long offset, long size, DataSink sink) throws IOException {
        ...
        for (DataSource src : mSources) {
            ...
            if (remaining >= size) {
                src.feed(offset, Math.toIntExact(size), sink);
                break;
            }

            // If the remaining is not enough, consume all.
            src.feed(offset, Math.toIntExact(remaining), sink);
            size -= remaining;
            offset = 0;
        }
    }
    ...
    public ByteBuffer getByteBuffer(long offset, int size) throws IOException {
        ...
        // Otherwise, read into a buffer.
        ByteBuffer buffer = ByteBuffer.allocate(size);
        mSources[i].copyTo(offset, Math.toIntExact(mSources[i].size() - offset), buffer);
        size -= mSources[i].size() - offset;
        for (i++; i < mSources.length; i++) {
            if (mSources[i].size() < size) {
                mSources[i].copyTo(0, Math.toIntExact(mSources[i].size()), buffer);
                size -= mSources[i].size();
            } else {
                mSources[i].copyTo(0, size, buffer);
                break;
            }
        }
        buffer.rewind();
    ...
    private Pair<Integer, Long> locateDataSource(long offset) {
        for (int i = 0; i < mSources.length; i++) {
            if (offset < mSources[i].size()) {
                return Pair.of(i, offset);
            }
            offset -= mSources[i].size();
        }
        return null;
    }
}
