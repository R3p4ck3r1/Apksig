 ...
 * The root hash will be used on device for on-access verification.  The tree itself is reproducible
 * on device, and will not be shipped with the APK.
 */
public class VerityTreeBuilder {

    /** Typical page size in bytes on Linux. */
    private final static int CHUNK_SIZE = 4096;

    /** Cryptographic hash algorithm currnetly supported. */
    ...
    public byte[] generateVerityTreeRootHash(DataSource fileSource) throws IOException {
        ...
        try {
            ...
            for (int i = levelOffset.length - 2; i >= 0; i--) {
                ...
                DataSink middleBufferSink = DataSinks.asDataSink(
                        slice(verityBuffer, levelOffset[i], levelOffset[i + 1]));

                DigestSink chunkedDigestSink =
                        new DigestSink(middleBufferSink, JCA_ALGORITHM, mSalt);
