...
public class VerityTreeBuilder {
    ...
    public byte[] generateVerityTreeRootHash(DataSource beforeApkSigningBlock,
        ...
        if (beforeApkSigningBlock.size() % CHUNK_SIZE != 0) {
            throw new IllegalStateException("APK Signing Block may not be page aligned.  Size " +
                    "before: " + beforeApkSigningBlock.size());
        }
        DataSource modifiedEocd = DataSources.asDataSource(
                EocdRecord.createWithCentralDirectoryFieldRemoved(
                        eocd.getByteBuffer(0, (int) eocd.size())));
