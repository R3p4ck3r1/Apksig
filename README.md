...
apksig library offers three primitives:
  * `(Default)ApkSignerEngine` which abstracts away signing an APK from parsing and building an APK
    ...
    which need to perform signing while building an APK, instead of after.
  * `ApkSigner` which signs the provided APK. ApkSigner's interface is much simpler than above
    `ApkSignerEngine`.
  * `ApkVerifier` which checks whether the provided APK is expected to verify on the provided Android
    platform versions.
...
apksigner command-line tool offers two operations:
  ...
  * check whether the provided APK is expected to verify on all Android platforms supported by the
    APK. Run `apksigner verify` for usage information.
